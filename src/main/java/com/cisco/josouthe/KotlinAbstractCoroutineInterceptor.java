    ...
    public Object onMethodBegin(Object objectIntercepted, String className, String methodName, Object[] params) {
        ...
        switch(methodName) {
            case "<init>": { //during init, constructor, we assume a BT is running, if not we start one, and then mark a handoff on this new object
                ...
                getLogger().info(String.format("Transaction markHandoff initiated for guid: '%s' isAsync Flag: %s Common Object: %s", transaction.getUniqueIdentifier(), transaction.isAsyncTransaction(), objectIntercepted));
                break;
            }
            case "start": { //once start method is executed, we begin processing this coroutine, so we want to start the segment here and store it for the callback on finish
                ...
                if (isFakeTransaction(transaction)) { //this object was not marked for handoff? log it
                    ...
                    getLogger().info(String.format("We intercepted an implementation of an AbstractCoroutine that was marked for handoff! coroutine: %s transaction segment guid: %s, %s %s.%s()", coroutineName, transaction.getUniqueIdentifier(), objectIntercepted, className, methodName));
                }
                transactionsMap.put(objectIntercepted, new TransactionDictionary(objectIntercepted, transaction));
                break;
            }
            case "onCancelled":
